import {
  Bluetooth,
  Cable,
  Camera,
  Car,
  Drone,
  Group,
  Handshake,
  Headphones,
  Joystick,
  Mail,
  MapPin,
  Mic,
  Monitor,
  Printer,
  ScanBarcode,
  ShieldCheck,
  ShoppingCartIcon,
  Smartphone,
  Watch,
} from "lucide-react";

export const navItems = [
  {
    id: 1,
    label: "Home",
    href: "/",
  },
  {
    id: 2,
    label: "Shop",
    href: "/shop",
  },
  {
    id: 3,
    label: "About Us",
    href: "/about",
  },
  {
    id: 4,
    label: "Contact",
    href: "/contact",
  },
];

export const categoryCardItems = [
  {
    id: 1,
    icon: Camera,
    label: "Camera",
  },
  {
    id: 2,
    icon: Headphones,
    label: "Headphones",
  },
  {
    id: 3,
    icon: Joystick,
    label: "Camera",
  },
  {
    id: 4,
    icon: Smartphone,
    label: "Camera",
  },
  {
    id: 5,
    icon: Monitor,
    label: "Camera",
  },
  {
    id: 6,
    icon: Watch,
    label: "Camera",
  },
  {
    id: 7,
    icon: Cable,
    label: "Camera",
  },
  {
    id: 8,
    icon: Printer,
    label: "Camera",
  },
  {
    id: 9,
    icon: Drone,
    label: "Camera",
  },
  {
    id: 10,
    icon: Bluetooth,
    label: "Camera",
  },
  {
    id: 11,
    icon: Mic,
    label: "Camera",
  },
  {
    id: 12,
    icon: ScanBarcode,
    label: "Camera",
  },
];

export const productItems = [
  {
    id: 1,
    img: "/images/product-01.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 2,
    img: "/images/product-1.png",
    name: "iPhone 15 Pro Max",
    price: "$1,199.99",
  },
  {
    id: 3,
    img: "/images/product-2.png",
    name: "AirPods Pro 2nd Gen",
    price: "$249.99",
  },
  {
    id: 4,
    img: "/images/product-01.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 5,
    img: "/images/product-4.png",
    name: "Canon EOS R6 Mark II",
    price: " $2,499.99",
  },
  {
    id: 6,
    img: "/images/product-5.png",
    name: "Apple Watch Series 9",
    price: "$399.99",
  },
  {
    id: 7,
    img: "/images/product-6.png",
    name: "Mechanical RGB Gaming Keyboard",
    price: "$149.99",
  },
  {
    id: 8,
    img: "/images/product-7.png",
    name: "DJI Mini 4 Pro Drone",
    price: "$759.99",
  },
];

export const newArrivalItems = [
  {
    id: 1,
    img: "/images/product-8.png",
    name: "VR Headset",
    price: "$499.99",
  },
  {
    id: 2,
    img: "/images/product-9.png",
    name: "Wireless Keyboard",
    price: "$99.99",
  },
  {
    id: 3,
    img: "/images/product-10.png",
    name: "Power Bank 24000mAh",
    price: "$149.99",
  },
  {
    id: 4,
    img: "/images/product-11.png",
    name: "Ring Video Doorbell",
    price: " $249.99",
  },
];

export const tabsTriggerItems = [
  {
    id: 1,
    icon: Camera,
    label: "Camera",
  },
  {
    id: 2,
    icon: Headphones,
    label: "Headphone",
  },
  {
    id: 3,
    icon: Smartphone,
    label: "Phone",
  },
  {
    id: 4,
    icon: Monitor,
    label: "computer",
  },
  {
    id: 5,
    icon: Watch,
    label: "Smart watch",
  },
  {
    id: 6,
    icon: Bluetooth,
    label: "Speaker",
  },
  {
    id: 7,
    icon: Joystick,
    label: "Controller",
  },
];

export const tabsContentItems = [
  {
    id: 1,
    value: "Camera",
    img: "/images/product-1.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 2,
    value: "Camera",
    img: "/images/product-1.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 3,
    value: "Camera",
    img: "/images/product-1.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 4,
    value: "Camera",
    img: "/images/product-1.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
];

export const testimonialsCardItems = [
  {
    id: 1,
    img: "/images/img-1.png",
    name: "James Blunt",
    role: "Businessman",
    text: "Lorem ipsum dolor sit amet consectetur. At commodo purus massa amet amet congue vulputate purus morbi. Congue facilisis tincidunt.",
  },
  {
    id: 2,
    img: "/images/img-2.png",
    name: "Jessica",
    role: "CEO",
    text: "Lorem ipsum dolor sit amet consectetur. At commodo purus massa amet amet congue vulputate purus morbi. Congue facilisis tincidunt.",
  },
];

export const footerList = [
  {
    id: 1,
    title: "shop by",
    links: [
      "Smart phone",
      "Desktop",
      "Laptop",
      "Camera",
      "Tablets",
      "Headphone",
      "Weekly Deals",
    ],
  },
  {
    id: 2,
    title: "Useful links",
    links: ["Home", "About", "Blog", "Contact"],
  },
  {
    id: 3,
    title: "Utility Pages",
    links: [
      "Style Guide",
      "Protected Guide",
      "404 Not Found",
      "License",
      "Changelog",
    ],
  },
];

export const priceItems = [
  {
    id: 1,
    title: "Price",
    list: [
      {
        label: "$0.00 -$99.99",
        qty: "5",
      },
      {
        label: "$100 -$199.99",
        qty: "5",
      },
      {
        label: "$200 and above",
        qty: "11",
      },
    ],
  },
  {
    id: 2,
    title: "Colors",
    list: [
      {
        label: "Deep blue",
        qty: "3",
      },
      {
        label: "Yellow",
        qty: "3",
      },
      {
        label: "Deep Grey",
        qty: "4",
      },
      {
        label: "Green",
        qty: "1",
      },
      {
        label: "Pure Black",
        qty: "7",
      },
      {
        label: "Pure white",
        qty: "4",
      },
    ],
  },
  {
    id: 3,
    title: "Size",
    list: [
      {
        label: "12.5 inch",
        qty: "5",
      },
      {
        label: "17.5 inch",
        qty: "5",
      },
      {
        label: "21.5 inch",
        qty: "4",
      },
    ],
  },
];

export const products = [
  {
    id: 1,
    img: "/images/product-01.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 2,
    img: "/images/product-1.png",
    name: "iPhone 15 Pro Max",
    price: "$1,199.99",
  },
  {
    id: 3,
    img: "/images/product-2.png",
    name: "AirPods Pro 2nd Gen",
    price: "$249.99",
  },
  {
    id: 4,
    img: "/images/product-01.png",
    name: "Business Ultrabook i7",
    price: "$1,499.99",
  },
  {
    id: 5,
    img: "/images/product-4.png",
    name: "Canon EOS R6 Mark II",
    price: " $2,499.99",
  },
  {
    id: 6,
    img: "/images/product-5.png",
    name: "Apple Watch Series 9",
    price: "$399.99",
  },
  {
    id: 7,
    img: "/images/product-6.png",
    name: "Mechanical RGB Gaming Keyboard",
    price: "$149.99",
  },
  {
    id: 8,
    img: "/images/product-7.png",
    name: "DJI Mini 4 Pro Drone",
    price: "$759.99",
  },
];

export const whyChooseUs = [
  {
    id: 1,
    icon: Car,
    title: "Shopping & Returns",
    text: " dictum mauris egestas. Eget libero curabitur.Lorem ipsum dolor sit amet consectetur. ",
  },
  {
    id: 2,
    icon: ShieldCheck,
    title: "Secure Shopping",
    text: " dictum mauris egestas. Eget libero curabitur.Lorem ipsum dolor sit amet consectetur. ",
  },
  {
    id: 3,
    icon: Handshake,
    title: "Affiliates",
    text: "dictum mauris egestas. Eget libero curabitur.Lorem ipsum dolor sit amet consectetur. ",
  },
  {
    id: 4,
    icon: Group,
    title: "Group Sale",
    text: " dictum mauris egestas. Eget libero curabitur.Lorem ipsum dolor sit amet consectetur. ",
  },
];

export const contactItems = [
  {
    id: 1,
    icon: ShoppingCartIcon,
    title: "Message",
    text: "non enim diam in volutpat. Egestas blandit dictum mauris egestas.",
  },
  {
    id: 2,
    icon: MapPin,
    title: "Address",
    text: "non enim diam in volutpat. Egestas blandit dictum mauris egestas.",
  },
  {
    id: 3,
    icon: Mail,
    title: "We’re Open",
    text: "non enim diam in volutpat. Egestas blandit dictum mauris egestas.",
  },
];
